...
export async function POST(
  ...
  if (!name || !identifier || menuItems?.length === 0) {
    return NextResponse.json(
      {
        type: ERROR_TYPE,
        message: "Please provide name, identifier and at least one menu item.",
      },
      { status: 400 },
    );
  }
  ...
  if (menuItems.find((item) => !item.label || !item.link)) {
    return NextResponse.json(
      {
        type: ERROR_TYPE,
        message: "Menu item title and link can not be empty",
      },
      { status: 400 },
    );
  }
  ...
  try {
    ...
    const incomingItemIds = menuItems
      .filter((item) => item.id)
      .map((item) => item.id!);
    ...
    await prisma.$transaction(async (tx) => {
      ...
      for (const item of toUpdateItems) {
        const existingItem = existingMenu.menuItems.find(
          (ei) => ei.id === item.id,
        );
        if (
          existingItem &&
          (item.label !== existingItem.label || item.link !== existingItem.link)
        ) {
          await tx.menuItem.update({
            where: { id: item.id },
            data: {
              label: item.label,
              code: toKebabCase(item.label),
              link: item.link,
            },
          });
        }
