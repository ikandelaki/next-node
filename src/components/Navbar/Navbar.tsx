...
export default function Navbar() {
  ...
  const renderSection = (
    ...
    return (
      <ul key={`${title}-${key}`}>
        ...
        {children[title].map((content: MenuItemType, key: number) =>
          renderNavLink(content, key),
        )}
      </ul>
    );
  };
  ...
  const renderSections = (children: { [key: string]: MenuItemType[] }) => {
    return Object.keys(children).map((title, key) =>
      renderSection(title, children, key),
    );
  };
  ...
  const renderCloseButton = () => {
    return (
      <div
        className="absolute right-5 w-6 h-6 cursor-pointer [&_path]:stroke-red-400"
        onClick={handleLinkClick}
      >
        <CloseIcon />
      </div>
  ...
  const renderExpandableSidebarContent = (children?: {
    ...
    return (
      <div
        className={`absolute w-full left-full top-0 bg-navbar-light h-full p-6 border-r border-line
                transition-translate transition-visiblity duration-300 ${
                  isExpanded
